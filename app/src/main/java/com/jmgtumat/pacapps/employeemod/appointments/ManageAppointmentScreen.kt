...
/**
 * Pantalla para gestionar las citas del empleado.
 *
...
fun ManageAppointmentsScreen(navController: NavController) {
    ...
    LaunchedEffect(empleadoId, selectedDate) {
        if (empleadoId != null) { // Solo se realiza la consulta si hay un ID de empleado válido
            empleadoId?.let { id ->
    ...
    EmpleadoDashboard(navController = navController) { innerPadding ->
        Column(
            ...
            DatePicker(selectedDate) { date ->
                selectedDate = date
                // Vuelve a obtener citas cuando cambia la fecha seleccionada
                empleadoId?.let { id ->
                    coroutineScope.launch {
                        citaViewModel.fetchCitasByEmpleadoIdAndDate(id, selectedDate.timeInMillis)
                    }
            ...
            if (citas.isEmpty()) {
                Text("No se encontraron citas para la fecha seleccionada.")
            } else {
                // Filtra las citas por ID de empleado y estado pendiente/confirmado
                val filteredCitas = citas.filter { it.empleadoId == empleadoId && (it.estado == CitaEstado.PENDIENTE || it.estado == CitaEstado.CONFIRMADA) }

                // Permite el desplazamiento vertical para listas largas de citas
...
/**
 * Componente composable que muestra una tarjeta de cita.
 *
 * @param cita La cita a mostrar.
 * @param citaViewModel El ViewModel de la cita.
            ...
            Box(
                ...
                Column {
                    ...
                    if (expanded) {
                        ...
                        // Oculta los botones para citas confirmadas
